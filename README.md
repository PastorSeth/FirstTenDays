...
Each day page is built in this order on purpose: **Bible Passage → MAPS questions → Video → Reflection Question.** There are two exceptions:

- **Day 1** uses its own "who wrote it / who was it for / why" questions in place of MAPS, since it's the intro day. It also uses a different order — **Video #1 → Bible Passage → Video #2 → Questions** — set via `flow: "video-passage-video"` on its entry in `data.js`, with `videoId` as Video #1 and `videoId2` as Video #2.
- **Day 2** uses a different order — **Video → MAPS → Reflection Question** (no Bible Passage section) — set via `flow: "video-first"` on that day's entry in `data.js`.

Both of those `flow` overrides are available on any other day too, by adding the same field to its entry in `data.js`.
